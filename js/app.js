...
async function cargarEstudiantesDesdeFirestore() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            const rawNombre = data.nombre || data.Nombre || 'Sin nombre';
            let rawPrograma = (data.programa || data.Programa || '').toString().toLowerCase().trim();
            if (rawPrograma.includes('fellow')) rawPrograma = 'fellow';
            if (rawPrograma.includes('residente')) rawPrograma = 'residente';
