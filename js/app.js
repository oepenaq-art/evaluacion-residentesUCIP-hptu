...
4. "Concepto Final": Un breve dictamen de cierre.`;
        ...
        // Intentar con los modelos más compatibles de Gemini en orden
        const modelosDisponibles = ['gemini-1.5-flash', 'gemini-2.0-flash', 'gemini-2.5-flash', 'gemini-1.5-pro'];
        let textoIA = null;
        let ultimoError = null;

        for (const modelo of modelosDisponibles) {
            try {
                const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${modelo}:generateContent?key=${encodeURIComponent(apiKey)}`, {
                    method: 'POST',
                ...
                if (response.ok) {
                    ...
                    let detalleGoogle = `Código ${response.status}`;
                    try {
                        const errJson = await response.json();
                        if (errJson.error?.message) {
                            detalleGoogle = errJson.error.message;
                        }
                    } catch (_) {}

                    ultimoError = new Error(`[${modelo}] ${detalleGoogle}`);
                    // Si el modelo no existe (404), prueba el siguiente
                    if (response.status === 404) {
                        console.warn(`Modelo ${modelo} no disponible (404). Probando siguiente...`);
                        continue;
                    }
                    // Si es otro error (por ejemplo 400 API Key inválida o 403 de cuota), frena para mostrar el error real
                    throw ultimoError;
                }
            } catch (errLoop) {
                ultimoError = errLoop;
                if (errLoop.message && errLoop.message.includes('404')) {
                    continue;
                }
                throw errLoop;
            }
        ...
        if (!textoIA) {
            throw ultimoError || new Error('No se pudo obtener respuesta de ningún modelo de Gemini.');
        }
        ...
        Swal.fire('Error al consultar Gemini', error.message || 'No se pudo generar el informe.', 'error');
    } finally {
        btnGenerarIa.innerHTML = '<span>🔍</span> Generar Informe y Calcular Notas';
        btnGenerarIa.disabled = false;
    }
