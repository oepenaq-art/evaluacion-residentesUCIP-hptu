    ...
    if (esRonda) {
        headerHtml = `
            <thead class="text-white text-xs uppercase font-bold sticky top-0">
                <tr>
                    <th class="p-3 bg-blue-700 border border-blue-900 text-left style="min-width: 140px;"">Ítem</th>
                    <th class="p-3 bg-red-600 border border-red-800 text-center style="min-width: 130px;"">Insuficiente 0</th>
                    <th class="p-3 bg-orange-600 border border-orange-800 text-center style="min-width: 130px;"">Por Mejorar 1.0 - 2.9</th>
                    <th class="p-3 bg-blue-600 border border-sky-800 text-center style="min-width: 130px;"">Aprobado 3.0 - 4.0</th>
                    <th class="p-3 bg-green-600 border border-green-800 text-center style="min-width: 130px;"">Sobresaliente 4.1 - 5.0</th>
                </tr>
            </thead>
        `;
    } else {
        headerHtml = `
            <thead class="text-white text-xs uppercase font-bold sticky top-0">
                <tr>
                    <th class="p-3 bg-blue-700 border border-blue-900 text-left style="min-width: 140px;"">Ítem</th>
                    <th class="p-3 bg-red-600 border border-red-800 text-center style="min-width: 130px;"">Insuficiente 1.0</th>
                    <th class="p-3 bg-orange-600 border border-orange-800 text-center style="min-width: 130px;"">Por Mejorar 2.0</th>
                    <th class="p-3 bg-blue-600 border border-sky-800 text-center style="min-width: 130px;"">Aprobado 3.0</th>
                    <th class="p-3 bg-green-600 border border-green-800 text-center style="min-width: 130px;"">Sobresaliente 4.0</th>
                    ${tieneNivel5 ? '<th class="p-3 bg-green-700 border border-emerald-900 text-center style="min-width: 130px;"">Excelente 5.0</th>' : ''}
                </tr>
            </thead>
        `;
    }
...
function renderizarFormulario() {
    ...
    dataRubrica.items.forEach(item => {
        ...
        if (esRonda) {
            ...
            inputNotaHtml = `
                <input type="hidden" id="input_nota_${item.id}" name="nota_${item.id}" value="" class="nota-hidden-req">
            `;
        }
        ...
        const itemCardHtml = `
            <div class="border rounded-xl p-5 bg-white shadow-sm border-gray-200" id="card_item_${item.id}">
                ...
                <div class="text-xs text-gray-600 bg-white p-3 rounded-lg border border-gray-100 leading-relaxed shadow-inner">
...
formEvaluacion.addEventListener('submit', async (e) => {
    ...
    // Validación personalizada para inputs ocultos (Seminarios y Tema Central)
    const hiddenInputs = document.querySelectorAll('.nota-hidden-req');
    for (let input of hiddenInputs) {
        if (!input.value) {
            Swal.fire('Faltan datos', 'Por favor, asegúrate de seleccionar una calificación para todos los ítems de la rúbrica.', 'warning');
            return;
        }
    }
...
btnInformes.addEventListener('click', () => {
    ...
    // Cargar clave guardada si existe en el navegador
    const claveGuardada = localStorage.getItem('gemini_api_key_hptu');
...
btnGenerarIa.addEventListener('click', async () => {
    ...
    localStorage.setItem('gemini_api_key_hptu', apiKey);
    ...
    try {
        ...
        const promptText = `Actúa como el Coordinador Académico de la Unidad de Cuidado Intensivo Pediátrico del Hospital Pablo Tobón Uribe.
...
4. Conecta el desempeño real del residente (notas y comentarios) explícitamente con las competencias esperadas en Cuidado Intensivo Pediátrico.
...
6. NO incluyas saludos, títulos, HTML ni despedidas, ve directo al texto del informe en texto plano.`;
        ...
        // 1. OBTENER MODELOS DISPONIBLES DINÁMICAMENTE Y ORDENARLOS POR VERSIÓN
        ...
        // Filtrar modelos que soporten generación y sean de gemini
        ...
        if (availableModels.length === 0) {
            throw new Error('La llave es válida, pero no tiene modelos de IA habilitados en este proyecto.');
        }

        // Ordenar alfabéticamente descendente para probar siempre las versiones más nuevas primero (ej. 3.1 antes que 2.5)
        availableModels.sort((a, b) => b.name.localeCompare(a.name));

        // 2. INTENTAR GENERAR EL REPORTE CON EL MODELO MÁS RECIENTE
        let textoIA = null;
        ...
        if (!textoIA) {
            throw ultimoError || new Error('No se pudo generar el texto con ninguno de los modelos disponibles.');
        }
        
        // Formatear texto IA (separar párrafos)
        ...
        infTexto.innerHTML = `
            <div style="font-family: 'Inter', Arial, sans-serif; color: #333; padding: 20px 30px; background: white; border-radius: 8px;">
                ...
                <div style="text-align: center; margin-bottom: 30px;">
                    <h2 style="color: #005A9C; margin: 0; font-size: 22px; font-weight: bold;">HOSPITAL PABLO TOBÓN URIBE</h2>
                    <h3 style="color: #333; margin: 5px 0; font-size: 16px;">Unidad de Cuidado Intensivo Pediátrico</h3>
                    <p style="color: #666; margin: 0; font-size: 13px;">Programa de Especialización en Pediatría / Cuidado Crítico</p>
        ...
        // DESCARGAR EL WORD AUTOMÁTICAMENTE
        try {
            lastWordData = [
                est,
...
/**
 * Función auxiliar para generar y descargar documento Word
 */
async function descargarInformeWord(est, rotacion, fechas, notas, textoIA, comentariosDocentes, modeloUsado) {
    ...
    // Intentar cargar logo (puede fallar por CORS o ruta, es opcional)
    let logoBuffer = null;
    try {
    ...
    headerChildren.push(
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 }, children: [
            new TextRun({ text: 'HOSPITAL PABLO TOBÓN URIBE', bold: true, size: 30, color: '005A9C' })
        ]}),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 }, children: [
            new TextRun({ text: 'Unidad de Cuidado Intensivo Pediátrico', size: 24, color: '333333' })
        ]}),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 300 }, children: [
            new TextRun({ text: 'Programa de Especialización en Pediatría / Cuidado Crítico', size: 22, color: '666666' })
        ]}),
    ...
    // Convertir saltos de linea en comentarios a múltiples Paragraphs si aplica
    ...
    const firmas = [
        ...
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 100 }, children: [new TextRun({ text: 'Coordinador Académico UCIP', bold: true, size: 22 })] }),
        new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'Hospital Pablo Tobón Uribe', size: 20, color: '666666' })] }),
    ...
    a.href = url;
    a.download = `Informe_Final_${est.nombre.replace(/\s+/g, '_')}_${rotacion.replace(/\s+/g, '_')}.docx`;
...
// Guardamos los últimos datos generados en variables globales para poder re-descargar el Word
let lastWordData = null;

// Evento para el botón de descargar Word manualmente
    ...
    const modalHtml = `
        <div class="text-left space-y-4">
            <div class="bg-gray-50 p-3 rounded border">
                <h4 class="font-bold text-sm text-blue-700 mb-2">➕ Agregar Nuevo Estudiante</h4>
