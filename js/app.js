...
4. "Concepto Final": Un breve dictamen de cierre.`;
        ...
        console.log("Modelos disponibles ordenados:", availableModels.map(m => m.name));
        ...
        for (const modelObj of availableModels) {
            ...
            try {
                console.log(`Intentando con el modelo: ${selectedModel}`);
                ...
                if (response.ok) {
                    ...
                    // Si el modelo da 404 o dice que ya no está disponible, probamos el siguiente de la lista
                    if (response.status === 404 || detalleGoogle.toLowerCase().includes('no longer available')) {
                        console.warn(`El modelo ${selectedModel} ya no está disponible. Intentando el siguiente...`);
                        continue; 
                    }
                    // Si es un error de permisos o cuota, detenemos el proceso
                    throw ultimoError;
                }
            } catch (errLoop) {
                ultimoError = errLoop;
                if (errLoop.message && (errLoop.message.includes('404') || errLoop.message.toLowerCase().includes('no longer available'))) {
                    continue;
                }
                throw errLoop;
            }
        ...
        infTexto.innerHTML = `
            <div class="mb-4 text-center">
                ...
                <p class="text-xs text-gray-400 mt-2">Modelo usado: ${selectedModel.replace('models/', '')}</p>
            </div>
            ${cleanHtml}
        `;
